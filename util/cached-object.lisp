...
Returns ID and CLASS which can be passed to GET-OBJ later."
  ...
  (let ((class (class-of object))
        (id (id-of object)))
    (multiple-value-bind (object-cache found-p) (object-cache-of class)
      (if found-p
          (setf (gethash id object-cache) object)
          (setf (object-cache-of class) (setf object-cache (mk-object-cache class))
...
CLASS can be a symbol or a CLASS type."
  ...
  (multiple-value-bind (object-cache found-p) (object-cache-of class)
    (if found-p
        (multiple-value-bind (obj found-p) (gethash id object-cache)
          (if found-p
              (values obj t t)
              (values nil nil t)))
        (values nil nil nil))))
...
(defmethod initialize-instance :around ((object cached-object) &key (avoid-gc nil avoid-gc-supplied-p))
  ...
  (if avoid-gc-supplied-p
      (cache-object object :avoid-gc avoid-gc)
      (cache-object object)))
